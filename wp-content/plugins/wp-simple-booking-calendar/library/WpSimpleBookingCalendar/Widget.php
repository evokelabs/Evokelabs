...
class WpSimpleBookingCalendar_Widget extends WP_Widget
{
	 ...
	 * @var array
	 */
	protected $_supportedParams = array('id', 'title', 'legend', 'view', 'dropdown', 'weekstart');
	...
	{
		$defaults = array('title' => 'yes', 'legend' => 'yes', 'view' => 1, 'dropdown' => 'yes', 'weekstart' => '0');
		foreach ($this->_supportedParams as $key)
		{
			$defaults[$key] = (isset($this->_defaults[$key]) ? $this->_defaults[$key] : '');
		}
        if($defaults['weekstart']==7) $defaults['weekstart']=0;
	...
	public function widget($args, $instance)
	{
		...
		$view->setTemplate('widget/widget')
			->assign('widget', $this)
			->assign('showLegend', $settings['legend'])
			->assign('showTitle', $settings['title'])
			->assign('calendarId', $settings['id'])
            ->assign('calendarView', $settings['view'])
            ->assign('enableDropdown', $settings['dropdown'])
            ->assign('weekStart', $settings['weekstart'])
			->assign('calendar', $model->getCalendarById($settings['id']))
			->assign('args', $args)
			->render();
	}
}