...
jQuery(function ($) {
  $(document).on('click', '#updateCalendarMultiple', function (e) {
    ...
    var startTime =
      Date.parse(startDay + ' ' + startMonth + ' ' + startYear) / 1000
    ...
    if (startTime < endTime) {
      for (i = startTime; i <= endTime; i = i + 60 * 60 * 24) {
        ...
        if (
          ...
          $('select.day' + changeDate.getDate())
            .find('option.sbc-status-' + status)
          ...
          $('select.day' + changeDate.getDate())
            ...
            .find('.sbc-editor-day')
            .removeClass()
            .addClass('sbc-editor-day sbc-status-' + status)
          $('.sbc-day' + changeDate.getDate())
            .removeClass()
            .addClass(
              'sbc-day' + changeDate.getDate() + ' sbc-status-' + status
            )
        }
  ...
  function getCurrentMonth() {
    var date = $('.sbc-navigation select[name="sbcdatepicker"]')
      .val()
      .split('/')
  ...
  $editCalendarForm.bind('submit', function () {
    $('input[name="calendarData"]', $editCalendarForm).val(
      JSON.stringify(sbcCalendarData)
    )
  ...
  $(document).on('change', '#sbc-editor select', function () {
    ...
    $this
      .closest('tr')
      .children('td.sbc-editor-day')
      .attr('class', 'sbc-editor-day ' + statusClass)
